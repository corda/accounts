...
class AccountKeysTests {
    ...
    fun `should create multiple keys for an account when requested`() {
        ...
        val account1 = a.startFlow(OpenNewAccountFlow("Stefano_Account1")).let {
            network.runNetwork()
            it.getOrThrow()
        }

        val account2 = a.startFlow(OpenNewAccountFlow("Stefano_Account2")).let {
            network.runNetwork()
            it.getOrThrow()
        }
        ...
        val foundKeysForAccount1 = a.transaction {
            accountService.accountKeys(account1.state.data.accountId)
        }

        val foundKeysForAccount2 = a.transaction {
            accountService.accountKeys(account2.state.data.accountId)
        }
