...
import net.corda.accounts.service.AccountService
...
class ShareAccountTests {
    ...
    fun `should share a state and all required account info to party`() {
        val result = a.startFlow(OpenNewAccountFlow("Stefano_Account")).let {
            network.runNetwork()
            it.getOrThrow()
        }

        val ownedByAccountState = a.startFlow(IssueFlow(result.state.data.accountId)).let {
            network.runNetwork()
            it.getOrThrow()
        }
        ...
        b.transaction {
            Assert.assertThat(accountServiceOnB.accountInfo(result.state.data.accountId), `is`(result))
