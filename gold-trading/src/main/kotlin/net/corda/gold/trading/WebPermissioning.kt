...
class PermissionWebLoginToAccountFlow(val webAccount: String, val accountToPermission: UUID, val add: Boolean = true) : FlowLogic<WebAccountPermissioning>() {
    ...
    override fun call(): WebAccountPermissioning {
        return serviceHub.withEntityManager {
            val existingEntry = find(WebAccountPermissioning::class.java, webAccount) ?: WebAccountPermissioning(webAccount = webAccount, permissionedAccounts = listOf())
...
@Entity
@Table(name = "web_permissioning", indexes = [Index(name = "pk_idx", columnList = "web_account_name")])
@CordaSerializable
