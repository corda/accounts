...
class FundAdministratorController(@Autowired private val rpcConnection: NodeRPCConnection) {
    ...
    fun getAllLoansKnownAbout(request: HttpServletRequest): List<LoanBookView> {
        ...
        val validateSessionUser = validateSessionUser(request)
        val nodeParty = rpcConnection.proxy.nodeInfo().legalIdentities.first()
        val accountsIAmReceiverFor =
            getAllAccounts().filter { nodeParty in it.state.data.carbonCopyReceivers.map { it.accountHost } }.map { it.state.data }.map { it.signingKey }.toSet()

        return getAllLoans(validateSessionUser).filter { it.state.data.owningAccount in accountsIAmReceiverFor }.map { it.toLoanBookView() }
    }
    ...
    private fun getAllLoans(contextUser: String): List<StateAndRef<LoanBook>> {
        return rpcConnection.proxy.startFlow(::GetWebUserFlow, contextUser).returnValue.toCompletableFuture().thenCompose { user ->
            rpcConnection.proxy.startFlow(::GetLoansBroadcastToAccounts, user?.permissionedAccounts ?: listOf()).returnValue.toCompletableFuture()
        }.getOrThrow()
    }
